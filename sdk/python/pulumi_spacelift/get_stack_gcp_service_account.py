# coding=utf-8
# *** WARNING: this file was generated by pulumi-language-python. ***
# *** Do not edit by hand unless you're certain you know what you are doing! ***

import builtins as _builtins
import warnings
import sys
import pulumi
import pulumi.runtime
from typing import Any, Mapping, Optional, Sequence, Union, overload
if sys.version_info >= (3, 11):
    from typing import NotRequired, TypedDict, TypeAlias
else:
    from typing_extensions import NotRequired, TypedDict, TypeAlias
from . import _utilities

__all__ = [
    'GetStackGcpServiceAccountResult',
    'AwaitableGetStackGcpServiceAccountResult',
    'get_stack_gcp_service_account',
    'get_stack_gcp_service_account_output',
]

@pulumi.output_type
class GetStackGcpServiceAccountResult:
    """
    A collection of values returned by getStackGcpServiceAccount.
    """
    def __init__(__self__, id=None, module_id=None, service_account_email=None, stack_id=None, token_scopes=None):
        if id and not isinstance(id, str):
            raise TypeError("Expected argument 'id' to be a str")
        pulumi.set(__self__, "id", id)
        if module_id and not isinstance(module_id, str):
            raise TypeError("Expected argument 'module_id' to be a str")
        pulumi.set(__self__, "module_id", module_id)
        if service_account_email and not isinstance(service_account_email, str):
            raise TypeError("Expected argument 'service_account_email' to be a str")
        pulumi.set(__self__, "service_account_email", service_account_email)
        if stack_id and not isinstance(stack_id, str):
            raise TypeError("Expected argument 'stack_id' to be a str")
        pulumi.set(__self__, "stack_id", stack_id)
        if token_scopes and not isinstance(token_scopes, list):
            raise TypeError("Expected argument 'token_scopes' to be a list")
        pulumi.set(__self__, "token_scopes", token_scopes)

    @_builtins.property
    @pulumi.getter
    def id(self) -> _builtins.str:
        """
        The provider-assigned unique ID for this managed resource.
        """
        return pulumi.get(self, "id")

    @_builtins.property
    @pulumi.getter(name="moduleId")
    def module_id(self) -> Optional[_builtins.str]:
        """
        ID of the stack which uses GCP service account credentials
        """
        return pulumi.get(self, "module_id")

    @_builtins.property
    @pulumi.getter(name="serviceAccountEmail")
    def service_account_email(self) -> _builtins.str:
        """
        email address of the GCP service account dedicated for this stack
        """
        return pulumi.get(self, "service_account_email")

    @_builtins.property
    @pulumi.getter(name="stackId")
    def stack_id(self) -> Optional[_builtins.str]:
        """
        ID of the stack which uses GCP service account credentials
        """
        return pulumi.get(self, "stack_id")

    @_builtins.property
    @pulumi.getter(name="tokenScopes")
    def token_scopes(self) -> Sequence[_builtins.str]:
        """
        list of Google API scopes
        """
        return pulumi.get(self, "token_scopes")


class AwaitableGetStackGcpServiceAccountResult(GetStackGcpServiceAccountResult):
    # pylint: disable=using-constant-test
    def __await__(self):
        if False:
            yield self
        return GetStackGcpServiceAccountResult(
            id=self.id,
            module_id=self.module_id,
            service_account_email=self.service_account_email,
            stack_id=self.stack_id,
            token_scopes=self.token_scopes)


def get_stack_gcp_service_account(module_id: Optional[_builtins.str] = None,
                                  stack_id: Optional[_builtins.str] = None,
                                  opts: Optional[pulumi.InvokeOptions] = None) -> AwaitableGetStackGcpServiceAccountResult:
    """
    > **Note:** `StackGcpServiceAccount` is deprecated. Please use `GcpServiceAccount` instead. The functionality is identical.

    `StackGcpServiceAccount` represents a Google Cloud Platform service account that's linked to a particular Stack or Module. These accounts are created by Spacelift on per-stack basis, and can be added as members to as many organizations and projects as needed. During a Run or a Task, temporary credentials for those service accounts are injected into the environment, which allows credential-less GCP Terraform provider setup.

    ## Example Usage

    ```python
    import pulumi
    import pulumi_spacelift as spacelift

    # For a Module
    k8s_module = spacelift.get_stack_gcp_service_account(module_id="k8s-module")
    # For a Stack
    k8s_core = spacelift.get_stack_gcp_service_account(stack_id="k8s-core")
    ```


    :param _builtins.str module_id: ID of the stack which uses GCP service account credentials
    :param _builtins.str stack_id: ID of the stack which uses GCP service account credentials
    """
    __args__ = dict()
    __args__['moduleId'] = module_id
    __args__['stackId'] = stack_id
    opts = pulumi.InvokeOptions.merge(_utilities.get_invoke_opts_defaults(), opts)
    __ret__ = pulumi.runtime.invoke('spacelift:index/getStackGcpServiceAccount:getStackGcpServiceAccount', __args__, opts=opts, typ=GetStackGcpServiceAccountResult).value

    return AwaitableGetStackGcpServiceAccountResult(
        id=pulumi.get(__ret__, 'id'),
        module_id=pulumi.get(__ret__, 'module_id'),
        service_account_email=pulumi.get(__ret__, 'service_account_email'),
        stack_id=pulumi.get(__ret__, 'stack_id'),
        token_scopes=pulumi.get(__ret__, 'token_scopes'))
def get_stack_gcp_service_account_output(module_id: Optional[pulumi.Input[Optional[_builtins.str]]] = None,
                                         stack_id: Optional[pulumi.Input[Optional[_builtins.str]]] = None,
                                         opts: Optional[Union[pulumi.InvokeOptions, pulumi.InvokeOutputOptions]] = None) -> pulumi.Output[GetStackGcpServiceAccountResult]:
    """
    > **Note:** `StackGcpServiceAccount` is deprecated. Please use `GcpServiceAccount` instead. The functionality is identical.

    `StackGcpServiceAccount` represents a Google Cloud Platform service account that's linked to a particular Stack or Module. These accounts are created by Spacelift on per-stack basis, and can be added as members to as many organizations and projects as needed. During a Run or a Task, temporary credentials for those service accounts are injected into the environment, which allows credential-less GCP Terraform provider setup.

    ## Example Usage

    ```python
    import pulumi
    import pulumi_spacelift as spacelift

    # For a Module
    k8s_module = spacelift.get_stack_gcp_service_account(module_id="k8s-module")
    # For a Stack
    k8s_core = spacelift.get_stack_gcp_service_account(stack_id="k8s-core")
    ```


    :param _builtins.str module_id: ID of the stack which uses GCP service account credentials
    :param _builtins.str stack_id: ID of the stack which uses GCP service account credentials
    """
    __args__ = dict()
    __args__['moduleId'] = module_id
    __args__['stackId'] = stack_id
    opts = pulumi.InvokeOutputOptions.merge(_utilities.get_invoke_opts_defaults(), opts)
    __ret__ = pulumi.runtime.invoke_output('spacelift:index/getStackGcpServiceAccount:getStackGcpServiceAccount', __args__, opts=opts, typ=GetStackGcpServiceAccountResult)
    return __ret__.apply(lambda __response__: GetStackGcpServiceAccountResult(
        id=pulumi.get(__response__, 'id'),
        module_id=pulumi.get(__response__, 'module_id'),
        service_account_email=pulumi.get(__response__, 'service_account_email'),
        stack_id=pulumi.get(__response__, 'stack_id'),
        token_scopes=pulumi.get(__response__, 'token_scopes')))
